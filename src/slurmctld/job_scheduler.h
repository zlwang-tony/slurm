 ...
 *
 * job_ptr		The current job being evaluated, after it has gone
 * 			through the scheduling loop.
 * reject_arr_job	A pointer to the first job (array task) in the most
 * 			recently rejected array. If job_ptr belongs to the same
 * 			array and partition as reject_arr_job, then set
 * 			job_ptr's reason to match reject_arr_job.
 */
