...
static int _set_option_field(lua_State *L)
{
	...
	xlua_stack_dump("cli_filter/lua", "early _set_option_field", L);
...
static int _load_script(void)
{
        ...
        load = xlua_loadscript(L, "cli_filter/lua", lua_script_path, req_fxns, &load_time);
        if (load == L)
                return SLURM_SUCCESS;
        if (!load)
                return SLURM_ERROR;
...
extern int setup_defaults(slurm_opt_t *opt, bool early) {
	...
	xlua_stack_dump("cli_filter/lua", "setup_defaults, before lua_pcall", L);
	...
	xlua_stack_dump("cli_filter/lua", "setup_defaults, after lua_pcall", L);
...
extern int pre_submit(slurm_opt_t *opt, int pack_offset)
{
	...
	xlua_stack_dump("cli_filter/lua", "pre_submit, before lua_pcall", L);
	...
	xlua_stack_dump("cli_filter/lua", "pre_submit, after lua_pcall", L);
...
extern int post_submit(int pack_offset, uint32_t jobid, uint32_t stepid)
{
	...
	xlua_stack_dump("cli_filter/lua", "post_submit, before lua_pcall", L);
	...
	xlua_stack_dump("cli_filter/lua", "post_submit, after lua_pcall", L);
