...
static int _load_script(void)
{
	...
	load = xlua_loadscript(L, "job_submit/lua", lua_script_path, req_fxns, &load_time);
	if (load == L)
		      ...
		      char **err_msg)
{
	...
	xlua_stack_dump("job_submit/lua", "job_submit, before lua_pcall", L);
	...
	xlua_stack_dump("job_submit/lua", "job_submit, after lua_pcall", L);
	if (user_msg) {
		*err_msg = user_msg;
		user_msg = NULL;
	}
		      ...
		      uint32_t submit_uid)
{
	...
	lua_pushnumber (L, submit_uid);
	xlua_stack_dump("job_submit/lua", "job_modify, before lua_pcall", L);
	...
	xlua_stack_dump("job_submit/lua", "job_modify, after lua_pcall", L);
	if (user_msg) {
